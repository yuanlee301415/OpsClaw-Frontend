/*
 * Markdown 脚注插件
 * Forked form: [markdown-it-footnote@v4.0.0](https://github.com/markdown-it/markdown-it-footnote/blob/master/index.mjs)
 *
 * 二开原因：
 * - 原始版本使用了 HTML a 标签的锚点功能，与 vue-router hash 冲突
 * - 修复方案：不使用 HTML a 标签，使用 scrollIntoView() 方法，实现锚点功能
 */
'use strict'
...
function render_footnote_ref(tokens, idx, options, env, slf) {
  ...
  return `<sup class="footnote-ref"><span id="fnref${refid}" onclick="document.querySelector('#fn${id}').scrollIntoView({behavior: 'smooth', block: 'center'})">${caption}</span></sup>`
}
...
function render_footnote_anchor(tokens, idx, options, env, slf) {
  ...
  return ` <span class="footnote-backref" onclick="document.querySelector('#fnref${id}').scrollIntoView({behavior: 'smooth', block: 'center'})">\u21a9\uFE0E</span>`
}
