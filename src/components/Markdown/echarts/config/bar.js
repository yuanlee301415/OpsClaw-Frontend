 ...
 * @param {{data: number[], name: string}[]} config.data Y轴数据
 ...
 * @param {boolean} [config.reverse=false] 是否反转XY轴位置(可选，默认为不反转以柱状图显示，设置反转后为条形图显示)
 */
export default function bar(config) {
  const { title, category, data, tooltip = true, unit = ['', ''], yRange: yRange = [], reverse } = { ...config }
  const categoryAxis = {
    type: 'category',
    data: category,
    axisLabel: {
  ...
  return {
    ...
    xAxis: reverse ? valueAxis : categoryAxis,
    yAxis: reverse ? categoryAxis : valueAxis,
    series:
      data &&
      data.map((_) => ({
        name: _.name,
